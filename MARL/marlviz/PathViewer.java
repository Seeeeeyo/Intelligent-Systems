...
public class PathViewer extends JPanel{
	...
	public void paintComponent(Graphics g) {
		...
		if (currentPath.size()>0) {
			...
			g2d.drawOval(getXFor(drawclone.get(drawclone.size()-1)[0])-2, getYFor(drawclone.get(drawclone.size()-1)[1])-2, 4, 4);
		}
		
		g2d.setColor(Color.black);
		g2d.drawString("Agent 0", getXFor(0.45), getYFor(-0.05));
		AffineTransform defaultAt = g2d.getTransform();
        // Rotates the coordinate by 90 degree counterclockwise to write vertically
        AffineTransform at = new AffineTransform();
        at.rotate(- Math.PI / 2);
        g2d.setTransform(at);
        g2d.drawString("1 tnegA", -getYFor(0.5), getXFor(-0.05)); //This is hilarious!!  MAC-OS-only??
